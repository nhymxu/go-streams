...
import (
	...
	"github.com/nhymxu/go-streams/adapters"
	"github.com/nhymxu/go-streams/extensions"
)
...
func NewController(hosts []string, options ...ControllerOption) (*Controller, error) {
	...
	controller := &Controller{
		...
		groupID:   adapters.DefaultQueueGroupID,
		hosts:     hosts,
		partition: 0,
		maxBytes:  10e6, // 10MB
	}
...
func (c *Controller) Subscribe(ctx context.Context, channel string) (extensions.BrokerChannelSubscription, error) {
	...
	sub := extensions.NewBrokerChannelSubscription(
		make(chan extensions.BrokerMessage, adapters.BrokerMessagesQueueSize),
		make(chan any, 1),
	)
